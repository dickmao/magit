...
- New transient ~magit2-sparse-checkout~ provides an interface to the
  ~git sparse-checkout~ command, introduced in Git v2.25.  #4102

- New command ~magit2-clone-sparse~ makes it possible to clone a
  repository and then immediately enable a sparse checkout, avoiding a
  checkout of the full working tree.  #4102

- The ~magit2-clone~ transient now includes ~--filter~ (hidden by
...
- The helper function ~magit2-current-blame-chunk~ passed the directory
  visited by a Dired buffer to ~git blame~, leading to some commands
  (e.g., ~magit2-checkout~) erroring when called from a Dired buffer.
  #4560

- For Helm users, calling ~magit2-stash-drop~ on the "Stashes" section
  has been broken since v3.0.0.  #4571

- ~magit2-discard~ didn't properly handle conflicts resulting from a
  file being renamed on both sides.  #4225

- Since v3.0.0, ~magit2-pull-from-pushremote~ has failed on
