...
Also see https://emacsair.me/2021/08/01/magit2-3.2.

** Changes since v3.1.0

- The default of ~magit2-bury-buffer-function~ was changed to
  ~magit2-mode-quit-window~.  #4412

- Added new option ~magit2-remote-git-executable~ alongside the existing
  ~magit2-git-executable~.  Previously the latter was used for local and
...
- We now use an absolute path for ~magit2-git-executable~ on macOS as
...
- The default ~magit2-pop-revision-stack-format~ now include the
  committer date. 1f85297a3

- The default ~magit2-log-revision-headers-format~ now includes notes.
  33bd79fd45

- Added new options ~magit2-repolist-sort-key~
  and ~magit2-submodule-list-sort-key~.  #4409

- Added new function ~magit2-repolist-column-flags~.  35067f512

- Added new option ~magit2-ediff-use-indirect-buffers~.  #4395

- Added new function ~magit2-process-git~.  fd5cf6173

- Added new function ~magit2-git-config-p~.  60f82a323

** Fixes since v3.1.0

- In the normal case ~magit2-name-tag~ failed to return the tag.
  14170b4ea

- ~magit2-completing-read-multiple*~ tried to use a variable that is
...
- ~magit2-git-shortlog~ got stuck in the repository in which it was first
  invoked.  384610edb

- ~magit2-commit-augment~ did not support ~--allow-empty~.  bbdfd4287

- Squash commands didn't fully take ~--author~ into account.  02d2917c6

- ~magit2-completing-read-multiple*~ conflicted with the ~consult~ package.
  #4437
