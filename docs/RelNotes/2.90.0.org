...
Also see https://emacsair.me/2018/11/08/magit2-2.90.
...
- Many commands were renamed, making their names longer.  The old
  ...
  part change.  In this release we rename, for example, ~magit2-tag~ to
  ~magit2-tag-create~ and in a later release ~magit2-tag-popup~ will be
  renamed to ~magit2-tag~.  #3619

- Removed many obsolete variable and function aliases.

- Removed many obsolete variables that are not mere aliases.

- Removed ~magit2-section~ accessor functions.  Use the ~oref~ and ~oset~
  functions instead.

- The function ~magit2-section-when~ has been deprecated in favor
  of ~magit2-section-match~ or the new ~magit2-section-value-if~.  The
...
- Added a new function ~magit2-completing-read-multiple*~, which is more
  like ~magit2-completing-read~ than ~magit2-completing-read-multiple~.
  Authors of extension should use the new function, which eventually
  will replace the old one.  cb4d59190

** Changes since v2.13.0

- Added a new mode ~magit2-wip-mode~, which enables automatic committing
  to work-in-progress refs whenever that makes sense.  Previously
  multiple ~magit2-wip-*~ modes had to be enabled to accomplish the same.
  These modes still exist, the new mode is implemented on top of them,
  but their explicit use by the user is discouraged.  #3534

- Added a new option ~magit2-wip-merge-branch~, which causes the current
...
- Added new commands ~magit2-wip-log-index~ and ~magit2-wip-log-worktree~.
  #3534
...
- It is now possible to specify the major-mode used to edit commit
  ...
  messages in ~magit2-revision-mode~ buffers.
...
- Added new option ~magit2-status-initial-section~, which controls which
...
- Added new commands ~magit2-commit-absorb~ and ~magit2-commit-absorb-popup~.
...
- ~magit2-file-rename~ now reads the target from the same directory as
  the source.  This is more convenient because removing parts of the
  path is easier than adding them again.

- Added new ~magit2-gitignore-popup~ and bound ~I~ to it, making it
  possible to use ~i~ for the ~magit2-imerge-popup~ from the ~magit2-imerge~
  package.

- Renamed the existing command ~magit2-describe-section~ to
  ~magit2-describe-section-briefly~ and added a new command
  ~magit2-describe-section~, which pops up a ~*Help*~ buffer,
  which contains information about the function and hook
  used to insert the section.  #3539

- Added new ~magit2-refname-pullreq~ face and a corresponding entry to
  ~magit2-ref-namespaces~.

- Added new option ~magit2-log-trace-definition-function~, which allows
  specifying the function that ~magit2-log-trace-definition~ should use
...
- Added new stub commands ~magit2-browse-thing~ and ~magit2-edit-thing~.
  Like other ~magit2-*-thing~ functions they are intend to be replaced
...
- ~magit2-merge-into~ now only offers local branches for completion and
...
- Added new variable ~magit2-perl-executable~ for platforms that put ~perl~
...
- Added new command ~magit2-reset-worktree~.  The existing ~magit2-reset-*~
...
- Improved ~magit2-process-password-auth-source~'s entry matching.
  60a335e5c

- Added new hook ~magit2-create-buffer~ hook.  2113dfc3d
...
- Added ~--graph~ to the default value of ~magit2-log-select-arguments~
...
- Added new command ~magit2-submodule-remove~ and the new related option
  ~magit2-submodule-remove-trash-gitdirs~.  The default is to not trash
  the gitdirs.  ~magit2-submodule-add~ now offers to reuse an existing
  gitdir when re-adding a submodule.  #3585

- ~magit2-submodule-add~ is run asynchronously now.  #3585
...
- Rebase commands that continue a rebase sequence can now use ~git rbr~
  ...
  you may use https://github.com/fanatoly/magit2-rbr.  c2cd780b2

- The manual is now also available in epub format, but it isn't very
  pretty.  #3579

- Added new hooks ~magit2-stage-hook~ and ~magit2-unstage-hook~.  #3496
...
- Added new hook ~git-commit-post-finish-hook~, a hook that has been
  ...
  when ~magit2-wip-merge-branch~ is non-nil.

- Added new hook ~magit2-post-commit-hook~, which is only run after
  running a ~magit2-commit-*~ command that does not require the user
  to edit the commit message.  #3615

- ~magit2-format-patch~ can now be told to only include the changes to
  certain files in the patch.  It now also refreshes Magit buffers.
  #3601

- The list of related refs in ~magit2-revision-mode~ buffers is no longer
  ...
  buffers using the option ~magit2-revision-insert-related-refs~.  #3487

- Added new function ~magit2-gitdir~ scheduled to replace ~magit2-git-dir~
  eventually because the latter has a really strange signature.
  5f407a29a

- Created new libraries from existing code.  9efcb1e8b ff

- The option ~magit2-revision-show-gravatar~ is now easier to customize.
  Customizing it to only show one of the two images actually works
  now.  f4b8fee66, b82228bc6

- The option ~magit2-repository-directories~ has a non-nil default value
  now.  f77d1158b

- The list of stashes in the status buffer is initially collapsed now.
  4dc1d0593

- The commands ~magit2-previous-line~ and ~magit2-next-line~ are used by
  default now.  3ae75c865

- Added support for ~--left-right~ output in logs.  #3627

- Added new option ~magit2-with-editor-envvar~.  Changing its value from
...
- Added new option ~magit2-clone-default-directory~.  #3635

- ~magit2-ref-namespaces~ is now taken into account when colorizing
  branch names in buffers listing refs.  #3645

- Added new option ~magit2-revision-fill-summary-line~.  #3610

** Fixes since v2.13.0

- A confirmation prompt during rebase was wrong.  37d76f409

- ~magit2-rebase-autosquash~ asked for confirmation when the user already
  confirmed earlier.  02e6c75b6

- ~magit2-commit-add-log~ inserted after the diff that gets inserted if
  ~commit.verbose~ is ~true~.  #3480

- ~magit2-log-wash-rev~ errored in ~magit2-cherry-mode~, starting with Emacs
  26.1.

- ~magit2-dired-jump~ failed, starting with Emacs 26.1.  #3469 
...
- In some edge cases ~magit2-branch-pull-request~ used to set Git
...
- ~magit2-gitignore-popup~'s autoload definition was broken.

- When using cygwin, then ~git-commit-setup-font-lock~ could end up
  trying to run git in a non-existent directory.  #3505

- ~magit2-completing-read~ did not protect the value of ~this-command~ even
  though there is code that assumes it does.  #3529

- ~magit2-read-file-trace~ in some cases claimed that valid user input
...
- ~magit2-merge-into~ signaled an error if the upstream of the current
  branch isn't configured instead of letting the user pick a target.
  #3550

- Some of the more complex ~magit2-cherry-*~ variants did not abort if
...
- ~magit2-file-rename~ failed to rename a file that is located at the
...
- If given one invalid rev, then ~magit2-rev-eq~ returned ~nil~, but when
  both revs are invalid, then it returned ~t~.  Now it returns ~nil~ in
  both cases.

- In an edge case ~magit2-diff-type~ falsely concluded that the
  uncommitted changes in a diff buffer already are committed.  #2627

- ~magit2--github-url-p~ failed to return ~t~ for an url for Github
  Enterprise instances.  #3572

- Trying to insert Gravatar images resulted in an error if the service
  is not reachable.  #3597

- ~magit2-display-buffer~ always selected the appropriate frame even if
  that was already selected.  That is undesirable because selecting
  a frame has side-effects.  f07eb85c6

- A key binding was not updated when ~magit2-submodule-deinit~ was renamed
  to ~magit2-submodule-unpopulate~.  b06845208

- ~magit2-submodule-visit~ could end up re-populating the visited module.
  3d794cd01

- The third-party ~git-annex~ program converts submodule gitdirs to
  symlinks, which we did not account for.  #3599

- ~magit2-bisect-start~ did fail silently when invalid user input, which
  flipped the good and bad revisions, instead of providing an
  informative error message.  #3604

- ~magit2-bisect-start~ did fail silently when there are uncommitted
...
- Autoloading ~magit2-submodules~ didn't work.  #3618

- Reverting files failed if the selection contained only binary files.
  c7bbe3e9a

- ~magit2-read-repository~ failed when ~magit2-repository-directories~ is
...
- When a local branch is being renamed, then ~magit2-branch-rename~ used
