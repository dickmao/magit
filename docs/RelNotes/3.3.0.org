...
Also see https://emacsair.me/2021/10/04/magit2-3.3.

** Breaking changes

- The last remaining alphabetic keys (both lower- and uppercase) are
  now bound in ~magit2-mode-map~.  ~H~ to ~magit2-describe-section~, ~j~ to the
  new ~magit2-status-quick~ (or ~magit2-status-jump~ in status buffers), ~J~
  to the new ~magit2-display-repository-buffer~, ~Q~ to ~magit2-git-command~
  and ~Z~ to ~magit2-worktree~, and once Forge is loaded, then ~N~ to
  ~forge-dispatch~.  (~Z~ used to be bound to ~magit2-stash~, which now is
  only available on ~z~.)  #4259, 9bec1c54a, e401bd4d3

- ~I~ is now bound to ~magit2-init~ instead of ~magit2-gitignore~, which
  continues to be available on ~i~.  Users of the ~magit2-imerge~ package
...
- ~magit2-stash-both~ now asks before discarding merge state.  #4345
...
- Added ~-Xignore-space-change~ and ~-Xignore-all-space~ to ~magit2-merge~.
  Like most newly added expert arguments they have to be explicitly
  [[https://magit2.vc/manual/transient/Enabling-and-Disabling-Suffixes.html][revealed]].  #4387

- Added new command ~magit2-status-quick~, which shows the status buffer
  but avoids refreshing it for performance reasons.  #4259

- Added new commands ~magit2-display-repository-buffer~ (bound to ~j~ in
  Magit buffers) and ~magit2-switch-to-repository-buffer~ (bound to ~J~)
  and variants for switching to any existing Magit buffer belonging
  to the current repository.  #4259

- Added new command ~magit2-dired-am-apply-patches~ for use in Dired.
  #4094.

- Rearranged ~magit2-patch~ and added a binding for ~magit2-am~.  8ec3a1a21,
  8cadf302b

- Added new command ~magit2-help~ (bound to ~h~ in ~magit2-dispatch~), which
...
- Bound ~imenu~ in the ~magit2-status-jump~ prefix.  678df30c3

- Added new transient prefix command ~magit2-bundle~, which provides
...
- Bound ~magit2-refs-set-show-commit-count~ in the ~magit2-show-refs~
  prefix.  2fe42f63d

- Added new option ~magit2-commit-diff-inhibit-same-window~.  #4132
...
- Added new command ~magit2-remote-unshallow~.  #4480

- Added new option ~git-commit-use-local-message-ring~.  #4503

** Fixes since v3.2.0

- Make ~magit2-branch-remote-head~ and ~magit2-branch-current~ fall back
...
- ~magit2-convert-filename-for-git~ did not expand absolute filenames,
  which is necessary because Git does not understand ~~/~.  de1dc8e74

- ~magit2-rebase~ did not enable ~--autostash~ by default but the
  documentation says it does.  421be65a3

- ~magit2-clone-read-repository~ did not convert filenames before passing
  them to Git.  4aa7d2928

- Due to a regression in v2.90.0, ~magit2-wip-log~ and
  ~magit2-wip-log-current~ failed to extract the previous tips of the
  wip refs from the reflog (which is relevant when
  ~~magit2-wip-merge-branch~ is nil).  c327824b0
