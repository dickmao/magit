...
#+texinfo_dir_title: Magit: (magit2).
...
#+begin_example
  M-x package-install RET magit2 RET
#+end_example
...
#+begin_src shell-script
  $ git clone https://github.com/magit2/magit2.git ~/.emacs.d/site-lisp/magit2
  $ cd ~/.emacs.d/site-lisp/magit2
#+end_src
...
Melpa or at ~/path/to/magit2/../<package>~, then you have to tell ~make~
where to find them.  To do so create the file ~/path/to/magit2/config.mk~
with the following content before running ~make~:

#+begin_src makefile
  LOAD_PATH  = -L ~/.emacs.d/site-lisp/magit2/lisp
...
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/site-lisp/magit2/lisp")
  (require 'magit2)

  (with-eval-after-load 'info
    (info-initialize)
    (add-to-list 'Info-directory-list
                 "~/.emacs.d/site-lisp/magit2/Documentation/"))
#+end_src
...
Instead of requiring the feature ~magit2~, you could load just the
autoload definitions, by loading the file ~magit2-autoloads.el~.

#+begin_src emacs-lisp
  (load "/path/to/magit2/lisp/magit2-autoloads")
#+end_src
...
that I can keep working on Magit.  See https://magit2.vc/donations.
...
buffers whose major-modes derive from ~magit2-mode~.

- Key: M-x magit2-toggle-buffer-lock ::
...
- Function: magit2-display-buffer buffer &optional display-function ::
  ...
  ~magit2-display-buffer-noselect~ is ~nil~.  It also runs the hooks
  mentioned below.
  ...
  by ~magit2-display-buffer-function~ is used.

- Variable: magit2-display-buffer-noselect ::

  When this is non-nil, then ~magit2-display-buffer~ only displays the
...
- User Option: magit2-display-buffer-function ::

  The function specified here is called by ~magit2-display-buffer~ with
...
- Function: magit2-display-buffer-traditional buffer ::

  This function is the current default value of the option
  ~magit2-display-buffer-function~.  Before that option and this function
...
- Function: magit2-display-buffer-same-window-except-diff-v1 ::

  This function displays most buffers in the currently selected
  window.  If a buffer's mode derives from ~magit2-diff-mode~ or
  ~magit2-process-mode~, it is displayed in another window.

- Function: magit2-display-buffer-fullframe-status-v1 ::

  This function fills the entire frame when displaying a status
  buffer.  Otherwise, it behaves like
  ~magit2-display-buffer-traditional~.

- Function: magit2-display-buffer-fullframe-status-topleft-v1 ::

  This function fills the entire frame when displaying a status
  buffer.  It behaves like ~magit2-display-buffer-fullframe-status-v1~
  except that it displays buffers that derive from ~magit2-diff-mode~
  or ~magit2-process-mode~ to the top or left of the current buffer
  rather than to the bottom or right.  As a result, Magit buffers tend
  to pop up on the same side as they would if
  ~magit2-display-buffer-traditional~ were in use.

- Function: magit2-display-buffer-fullcolumn-most-v1 ::
  ...
  window if (1) the buffer's mode derives from ~magit2-process-mode~,
  or (2) the buffer's mode derives from ~magit2-diff-mode~, provided
  that the mode of the current buffer derives from ~magit2-log-mode~ or
  ~magit2-cherry-mode~.

- User Option: magit2-pre-display-buffer-hook ::

  This hook is run by ~magit2-display-buffer~ before displaying the
  buffer.

- Function: magit2-save-window-configuration ::
...
- User Option: magit2-post-display-buffer-hook ::

  This hook is run by ~magit2-display-buffer~ after displaying the
  buffer.

- Function: magit2-maybe-set-dedicated ::
  ...
  information is later used by ~magit2-mode-quit-window~, to determine
  whether the window should be deleted when its last Magit buffer is
  buried.

*** Naming Buffers

- User Option: magit2-generate-buffer-name-function ::

  The function used to generate the names of Magit buffers.

  Such a function should take the options ~magit2-uniquify-buffer-names~
  as well as ~magit2-buffer-name-format~ into account.  If it doesn't,
  ...
  ~magit2-buffer-name-format~, then its own doc-string should describe
  the additions.

- Function: magit2-generate-buffer-name-default-function mode ::
  ...
  This function uses ~magit2-buffer-name-format~ and supporting all of
  the %-sequences mentioned the documentation of that option.  It also
  respects the option ~magit2-uniquify-buffer-names~.

- User Option: magit2-buffer-name-format ::
  ...
  - ~%M~
    ...
    Like ~%m~ but abbreviate ~magit2-status-mode~ as ~magit2~.
  ...
  - ~%t~
    ...
    if ~magit2-uniquify-buffer-names~ is non-nil an abbreviation of that.

  - ~%x~

    If ~magit2-uniquify-buffer-names~ is nil "*", otherwise the empty
  ...
  - ~%T~
    ...
    and only if ~magit2-uniquify-buffer-names~ is nil.

  The value should always contain ~%m~ or ~%M~, ~%v~ or ~%V~, and ~%t~ (or the
  obsolete ~%T~).  If ~magit2-uniquify-buffer-names~ is non-nil, then the
  value must end with ~%t~ or ~%t%x~ (or the obsolete ~%T~).  See issue
  #2841.

- User Option: magit2-uniquify-buffer-names ::
...
- User Option: magit2-bury-buffer-function ::

  The function used to actually bury or kill the current buffer.

  ~magit2-mode-bury-buffer~ calls this function with one argument.  If
  ...
  currently is ~magit2-restore-window-configuration~.

- Function: magit2-restore-window-configuration kill-buffer ::
...
- Function: magit2-mode-quit-window kill-buffer ::
...
#+begin_src emacs-lisp
  (with-eval-after-load 'magit2-mode
    (add-hook 'after-save-hook 'magit2-after-save-refresh-status t))
#+end_src
...
- Key: g (magit2-refresh) ::

  This command refreshes the current buffer if its major mode derives
  from ~magit2-mode~ as well as the corresponding status buffer.

  If the option ~magit2-revert-buffers~ calls for it, then it also
  reverts all unmodified buffers that visit files being tracked in the
  current repository.

- Key: G (magit2-refresh-all) ::
  ...
  ~magit2-revert-buffers~ is nil.

- User Option: magit2-refresh-buffer-hook ::
...
- User Option: magit2-refresh-status-buffer ::
...
- Function: magit2-after-save-refresh-status ::
...
doubt or after outside changes, type ~g~ (~magit2-refresh~) to save and
refresh explicitly.

- User Option: magit2-save-repository-buffers ::
...
- User Option: magit2-auto-revert-mode ::
...
- User Option: global-auto-revert-mode ::
  ...
  this mode /instead of/ ~magit2-auto-revert-mode~.

- User Option: magit2-auto-revert-immediately ::

  This option controls whether Magit reverts buffers immediately.

  If this is non-nil and either ~global-auto-revert-mode~ or
  ~magit2-auto-revert-mode~ is enabled, then Magit immediately reverts
  ...
  actually supported), then ~magit2-auto-revert-immediately~ does not
  have to be non-nil, because the reverts happen immediately anyway.

  If ~magit2-auto-revert-immediately~ and ~auto-revert-use-notify~ are both
...
- User Option: magit2-auto-revert-tracked-only ::

  This option controls whether ~magit2-auto-revert-mode~ only reverts
...
- User Option: auto-revert-mode ::
  ...
  The global mode ~magit2-auto-revert-mode~ works by turning on this
...
- User Option: auto-revert-buffer-list-filter ::
  ...
  setting the value to ~magit2-auto-revert-repository-buffer-p~.
...
- Key: n (magit2-section-forward) ::

  Move to the beginning of the next visible section.

- Key: M-p (magit2-section-backward-siblings) ::
...
- Key: M-n (magit2-section-forward-siblings) ::
...
- Key: ^ (magit2-section-up) ::

  Move to the beginning of the parent of the current section.

The above commands all call the hook ~magit2-section-movement-hook~.
...
#+begin_src emacs-lisp
  (with-eval-after-load 'magit2-diff
    (remove-hook 'magit2-section-movement-hook
                 'magit2-hunk-set-window-start))
#+end_src

- Variable: magit2-section-movement-hook ::

  This hook is run by all of the above movement commands, after
  arriving at the destination.

- Function: magit2-hunk-set-window-start ::
  ...
  Loading ~magit2-diff~ adds this function to the hook.

- Function: magit2-section-set-window-start ::

  This hook function ensures that the beginning of the current section
  is visible, regardless of the section's type.  If you add this to
  ~magit2-section-movement-hook~, then you must remove the hunk-only
  variant in turn.

- Function: magit2-log-maybe-show-more-commits ::
  ...
  Loading ~magit2-log~ adds this function to the hook.

- Function: magit2-log-maybe-update-revision-buffer ::
...
- Function: magit2-log-maybe-update-blob-buffer ::
...
- Function: magit2-status-maybe-update-revision-buffer ::

  When moving inside a status buffer, then this function updates the
  revision buffer, provided it is already being displayed in another
  window of the same frame.

- Function: magit2-status-maybe-update-stash-buffer ::

  When moving inside a status buffer, then this function updates the
  stash buffer, provided it is already being displayed in another
  window of the same frame.

- Function: magit2-status-maybe-update-blob-buffer ::
...
- Function: magit2-stashes-maybe-update-stash-buffer ::
...
- User Option: magit2-update-other-window-delay ::
...
- Key: TAB (magit2-section-toggle) ::

  Toggle the visibility of the body of the current section.

- Key: C-<tab> (magit2-section-cycle) ::

  Cycle the visibility of current section and its children.

- Key: M-<tab> (magit2-section-cycle-diffs) ::

  Cycle the visibility of diff-related sections in the current buffer.

- Key: S-<tab> (magit2-section-cycle-global) ::

  Cycle the visibility of all sections in the current buffer.

- Key: 1 (magit2-section-show-level-1) ::
- Key: 2 (magit2-section-show-level-2) ::
- Key: 3 (magit2-section-show-level-3) ::
- Key: 4 (magit2-section-show-level-4) ::

  Show sections surrounding the current section up to level N.

- Key: M-1 (magit2-section-show-level-1-all) ::
- Key: M-2 (magit2-section-show-level-2-all) ::
- Key: M-3 (magit2-section-show-level-3-all) ::
- Key: M-4 (magit2-section-show-level-4-all) ::
...
- Command: magit2-section-show ::

  Show the body of the current section.

- Command: magit2-section-hide ::

  Hide the body of the current section.

- Command: magit2-section-show-headings ::
...
~magit2-section-set-visibility-hook~.

- User Option: magit2-section-initial-visibility-alist ::
  ...
  Use the command ~magit2-describe-section-briefly~ to determine a
  ...
  can be used, see ~magit2-section-match~.

- User Option: magit2-section-cache-visibility ::
  ...
  This requires that the function ~magit2-section-cached-visibility~ is
  a member of ~magit2-section-set-visibility-hook~.

- Variable: magit2-section-set-visibility-hook ::
  ...
  not during refreshes.  If ~magit2-insert-section--oldroot~ is non-nil,
  then the buffer is being refreshed and these functions should
  immediately return ~nil~.

- User Option: magit2-section-visibility-indicator ::
  ...
  - ~(EXPANDABLE-BITMAP . COLLAPSIBLE-BITMAP)~
    ...
    ~left-fringe-width~ in ~magit2-mode-hook~, e.g.:

    #+BEGIN_SRC emacs-lisp
      (add-hook 'magit2-mode-hook (lambda ()
...
~magit2-describe-section~ can be used to see which hook (if any) was
responsible for inserting the section at point.

For buffers whose sections can be customized by the user, a hook
variable called ~magit2-TYPE-sections-hook~ exists.  This hook should be
changed using ~magit2-add-section-hook~.  Avoid using ~add-hooks~ or the
Custom interface.
...
- Function: magit2-add-section-hook hook function &optional at append local ::
...
~magit2-TYPE-section-map~.  That keymap is used as text property keymap
...
- User Option: magit2-section-show-child-count ::
...
used the package ~magit2-popup~ and even earlier versions library
~magit2-key-mode~.

Transient is documented in [[info:transient]].

- Key: C-c C-c (magit2-dispatch) ::
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-x M-g") 'magit2-dispatch)
#+end_src
...
prominently ~magit2-show-commit~ is usually invoked by typing ~RET~ while
point is on a commit in a log, but it can also be invoked from the
~magit2-diff~ transient prefix.
...
- User Option: magit2-prefix-use-buffer-arguments ::
  ...
  The ~magit2-diff~ and ~magit2-log~ transient prefix commands are affected
  by this option.

- User Option: magit2-direct-use-buffer-arguments ::
...
- The global diff and log arguments are set for each supported mode
  ...
  in ~magit2-diff-mode~, ~magit2-revision-mode~, ~magit2-merge-preview-mode~
  and ~magit2-status-mode~ buffers.  Setting or saving the value for one
  mode does not change the value for other modes.  The history however
  is shared.

- When ~magit2-show-commit~ is invoked directly from a log buffer, then
  the file filter is picked up from that buffer, not from the revision
  buffer or the mode's global diff arguments.

- Even though they are suffixes of the diff prefix ~magit2-show-commit~
  and ~magit2-stash-show~ do not use the diff buffer used by the diff
...
- If ~magit2-show-refs~ is invoked from a ~magit2-refs-mode~ buffer, then it
  ...
  respects ~magit2-prefix-use-buffer-arguments~.  If it is invoked
  elsewhere without a prefix argument, then it acts as a direct
  command and therefore respects ~magit2-direct-use-buffer-arguments~.
...
The option ~magit2-no-confirm~ can be used to tell Magit to perform
...
- User Option: magit2-no-confirm ::
  ...
  - Edit published history:
    ...
    - ~edit-published~ Affects the commands ~magit2-edit-line-commit~ and
      ~magit2-diff-edit-hunk-commit~.  These two commands make it quite
    ...
    set ~magit2-published-branches~ to ~nil~.
  ...
  - Global settings:
    ...
    When ~magit2-wip-before-change-mode~ is enabled, then the following
...
~magit2-dwim-selection~ can be used to configure certain commands to that
effect.
...
- User Option: magit2-dwim-selection ::
...
- Variable: magit2-keep-region-overlay ::
...
~magit2-completing-read-function~, which only controls the low-level
function used by ~magit2-completing-read~.  This option also makes it
possible to use a different completing mechanism for Magit than for
the rest of Emacs, but doing that is not recommend.

You most likely don't have to customize the magit2-specific option to
...
~magit2-ido-completing-read~ as ~magit2-completing-read-function~.

- User Option: magit2-completing-read-function ::

  The value of this variable is the low-level function used to perform
  completion by code that uses ~magit2-completing-read~ (as opposed to
  the built-in ~completing-read~).

  The default value, ~magit2-builtin-completing-read~, is suitable for
  ...
  suitable to be used here.  ~magit2-builtin-completing-read~ performs
  some additional work, and any function used in its place has to do
  the same.

- Function: magit2-builtin-completing-read prompt choices &optional predicate require-match initial-input hist def ::

  This function performs completion using the built-in ~completing-read~
  and does some additional magit2-specific work.

- Function: magit2-ido-completing-read prompt choices &optional predicate require-match initial-input hist def ::
  ...
  does some additional magit2-specific work.
...
- Function: magit2-completing-read prompt choices &optional predicate require-match initial-input hist def fallback ::
  ...
  the case depends on PROMPT, ~this-command~ and ~magit2-dwim-selection~.
  ...
  - FALLBACK specifies a secondary default that is only used if the
    ...
    to ~magit2-dwim-selection~ — if DEF is ~nil~ but FALLBACK is not, then
...
- User Option: magit2-list-refs-sortby ::
...
~magit2-process~ command when things don't go as expected.

The output/errors for up to `magit2-process-log-max' Git commands are
retained.
...
- Key: k (magit2-process-kill) ::

  This command kills the process represented by the section at point.

- Variable: magit2-git-debug ::
...
- Variable: magit2-process-extreme-logging ::

  This option controls whether ~magit2-process-file~ logs to the
  ~*Messages*~ buffer.
...
indicator in the mode line, using the ~magit2-mode-line-process~ face.
...
is instead highlighted with the ~magit2-mode-line-process-error~ face,
...
line until the next magit2 buffer refresh.

If you do not wish process errors to be indicated in the mode line,
customize the ~magit2-process-display-mode-line-error~ user option.
...
- Key: ! s (magit2-shell-command-topdir) ::

  This command reads a command from the user and executes it in the
  top-level directory of the current working tree.

- Key: ! S (magit2-shell-command) ::

  This command reads a command from the user and executes it in
  ~default-directory~.  With a prefix argument the command is executed
  in the top-level directory of the current working tree instead.

- User Option: magit2-shell-command-verbose-prompt ::
...
Windows version) or ~darwin~ (macOS) then ~magit2-git-executable~ is set
...
- User Option: magit2-git-executable ::
...
- User Option: magit2-remote-git-executable ::
...
The command ~magit2-debug-git-executable~ can be useful to find out where
Emacs is searching for ~git~.

- Key: M-x magit2-debug-git-executable ::

  This command displays a buffer with information about
  ~magit2-git-executable~ and  ~magit2-remote-git-executable~.

- Key: M-x magit2-version ::
...
- User Option: magit2-git-global-arguments ::
...
The command ~magit2-status~ displays the status buffer belonging to the
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-x g") 'magit2-status)
#+end_src

- Key: C-x g (magit2-status) ::
  ...
  directory, depending on the option ~magit2-repository-directories~, and
  ...
  - With a single prefix argument an existing repository is read from
    ...
    ~magit2-repository-directories~, then the behavior is the same as with
    two prefix arguments.

- User Option: magit2-repository-directories ::
  ...
  ~magit2-list-repositories~.  It also affects ~magit2-status~ (which see)
  in potentially surprising ways (see above).

- Command: magit2-status-quick ::

  This command is an alternative to ~magit2-status~ that usually avoids
  ...
  #+BEGIN_SRC emacs-lisp
    (global-set-key (kbd "C-x g") 'magit2-status-quick).
  #+END_SRC
...
- Function: magit2-insert-unstaged-changes ::

  Insert section showing unstaged changes.

- Function: magit2-insert-staged-changes ::

  Insert section showing staged changes.

- Function: magit2-insert-stashes &optional ref heading ::
...
- Function: magit2-insert-unpulled-from-upstream ::

  Insert section showing commits that haven't been pulled from the
  upstream branch yet.

- Function: magit2-insert-unpulled-from-pushremote ::

  Insert section showing commits that haven't been pulled from the
  push-remote branch yet.

- Function: magit2-insert-unpushed-to-upstream ::

  Insert section showing commits that haven't been pushed to the
  upstream yet.

- Function: magit2-insert-unpushed-to-pushremote ::
...
- Function: magit2-insert-tracked-files ::

  Insert a tree of tracked files.

- Function: magit2-insert-ignored-files ::
...
- Function: magit2-insert-skip-worktree-files ::
...
- Function: magit2-insert-assumed-unchanged-files ::
...
- Function: magit2-insert-unpulled-or-recent-commits ::

  Insert section showing unpulled or recent commits.
  If an upstream is configured for the current branch and it is
  ahead of the current branch, then show the missing commits.
  Otherwise, show the last ~magit2-log-section-commit-count~
  commits.

- Function: magit2-insert-recent-commits ::

  Insert section showing the last ~magit2-log-section-commit-count~
  commits.

- User Option: magit2-log-section-commit-count ::

  How many recent commits ~magit2-insert-recent-commits~ and
  ~magit2-insert-unpulled-or-recent-commits~ (provided there are no
  unpulled commits) show.

- Function: magit2-insert-unpulled-cherries ::

  Insert section showing unpulled commits.
  Like ~magit2-insert-unpulled-commits~ but prefix each commit
  that has not been applied yet (i.e. a commit with a patch-id
  not shared with any local commit) with "+", and all others
  with "-".

- Function: magit2-insert-unpushed-cherries ::

  Insert section showing unpushed commits.
  Like ~magit2-insert-unpushed-commits~ but prefix each commit
...
The contents of status buffers is controlled using the hook
~magit2-status-sections-hook~ (see [[*Status Sections]]).

By default ~magit2-insert-status-headers~ is the first member of that
hook variable.

- Function: magit2-insert-status-headers ::

  Insert headers sections appropriate for ~magit2-status-mode~ buffers.
  The sections are inserted by running the functions on the hook
  ~magit2-status-headers-hook~.

- User Option: magit2-status-headers-hook ::

  Hook run to insert headers sections into the status buffer.

  This hook is run by ~magit2-insert-status-headers~, which in turn has
  to be a member of ~magit2-status-sections-hook~ to be used at all.

By default the following functions are members of the above hook:

- Function: magit2-insert-error-header ::
...
- Function: magit2-insert-diff-filter-header ::

  Insert a header line showing the effective diff filters.

- Function: magit2-insert-head-branch-header ::

  Insert a header line about the current branch or detached ~HEAD~.

- Function: magit2-insert-upstream-branch-header ::

  Insert a header line about the branch that is usually pulled into
  the current branch.

- Function: magit2-insert-push-branch-header ::

  Insert a header line about the branch that the current branch is
  usually pushed to.

- Function: magit2-insert-tags-header ::
...
- Function: magit2-insert-repo-header ::

  Insert a header line showing the path to the repository top-level.

- Function: magit2-insert-remote-header ::
...
- Function: magit2-insert-user-header ::
...
By default ~magit2-insert-modules~ is /not/ a member of that hook
variable.

- Function: magit2-insert-modules ::

  Insert submodule sections.

  Hook ~magit2-module-sections-hook~ controls which module sections are
  inserted, and option ~magit2-module-sections-nested~ controls whether
  they are wrapped in an additional section.

- User Option: magit2-module-sections-hook ::

  Hook run by ~magit2-insert-modules~.

- User Option: magit2-module-sections-nested ::

  This option controls whether ~magit2-insert-modules~ wraps inserted
  sections in an additional section.

  If this is non-nil, then only a single top-level section is inserted.
  If it is nil, then all sections listed in ~magit2-module-sections-hook~
  become top-level sections.

- Function: magit2-insert-modules-overview ::
...
- Function: magit2-insert-modules-unpulled-from-upstream ::
...
- Function: magit2-insert-modules-unpulled-from-pushremote ::

  Insert sections for modules that haven't been pulled from the
  push-remote yet.  These sections can be expanded to show the
  respective commits.

- Function: magit2-insert-modules-unpushed-to-upstream ::

  Insert sections for modules that haven't been pushed to the upstream
  yet.  These sections can be expanded to show the respective commits.

- Function: magit2-insert-modules-unpushed-to-pushremote ::
...
- User Option: magit2-status-refresh-hook ::

  Hook run after a status buffer has been refreshed.

- User Option: magit2-status-margin ::
  ...
  - STYLE controls how to format the author or committer date.  It can
    ...
    ~magit2-log-margin-show-committer-date~ controls which date is being
    displayed.
...
- Command: magit2-list-repositories ::

  This command displays a list of repositories in a separate buffer.

  The options ~magit2-repository-directories~ and
  ~magit2-repository-directories-depth~ control which repositories are
  displayed.

- User Option: magit2-repolist-columns ::

  This option controls what columns are displayed by the command
  ~magit2-list-repositories~ and how they are displayed.
...
- Key: 5 (magit2-repolist-find-file-other-frame) ::
...
~magit2-log~, on ~l~, features several suffix commands, which show a
specific log in a separate log buffer.

Like other transient prefix commands, ~magit2-log~ also features several
...
log buffer, depending on the value of ~magit2-prefix-use-buffer-arguments~
...
- Key: l h (magit2-log-head) ::

  Show log for ~HEAD~.

- Key: l u (magit2-log-related) ::
...
- Key: l L (magit2-log-branches) ::

  Show log for all local branches and ~HEAD~.

- Key: l b (magit2-log-all-branches) ::

  Show log for all local and remote branches and ~HEAD~.

- Key: l a (magit2-log-all) ::
...
Visiting Files]].  The command ~magit2-cherry~ also shows a log, see
[[*Cherries]].

*** Refreshing Logs

The transient prefix command ~magit2-log-refresh~, on ~L~, can be used to
...
- Key: L (magit2-log-refresh) ::

  This transient prefix command binds the following suffix commands
  along with the appropriate infix arguments and displays them in a
  temporary buffer until a suffix is invoked.

- Key: L g (magit2-log-refresh) ::

  This suffix command sets the local log arguments for the current
  buffer.

- Key: L s (magit2-log-set-default-arguments) ::
...
- Key: L w (magit2-log-save-default-arguments) ::
...
- Key: q (magit2-log-bury-buffer) ::

  Bury the current buffer or the revision buffer in the same frame.
  Like ~magit2-mode-bury-buffer~ (which see) but with a negative prefix
  argument instead bury the revision buffer, provided it is displayed
  in the current frame.

- Key: C-c C-b (magit2-go-backward) ::

  Move backward in current buffer's history.

- Key: C-c C-f (magit2-go-forward) ::

  Move forward in current buffer's history.

- Key: C-c C-n (magit2-log-move-to-parent) ::
...
- Key: j (magit2-log-move-to-revision) ::
...
- Key: SPC (magit2-diff-show-or-scroll-up) ::
...
- Key: DEL (magit2-diff-show-or-scroll-down) ::
...
- Key: = (magit2-log-toggle-commit-limit) ::
...
- Key: + (magit2-log-double-commit-limit) ::

  Double the number of commits the current log buffer is limited to.

- Key: - (magit2-log-half-commit-limit) ::

  Half the number of commits the current log buffer is limited to.

- User Option: magit2-log-auto-more ::
...
- User Option: magit2-log-show-refname-after-summary ::
...
using the faces named ~magit2-signature-*~ (which see).

For a description of ~magit2-log-margin~ see [[*Log Margin]].
...
used to configure the margin are named ~magit2-INFIX-margin~, where INFIX
is the same as in the respective major-mode ~magit2-INFIX-mode~.  In
regular log buffers that would be ~magit2-log-margin~.

- User Option: magit2-log-margin ::
...
You can change the STYLE and AUTHOR-WIDTH of all ~magit2-INFIX-margin~
options to the same values by customizing ~magit2-log-margin~ *before*
~magit2~ is loaded.  If you do that, then the respective values for the
other options will default to what you have set for that variable.
Likewise if you set INIT in ~magit2-log-margin~ to ~nil~, then that is used
...
- User Option: magit2-log-margin-show-committer-date ::
...
~magit2-log-refresh~, but that transient features the same commands, and
then some other unrelated commands.

- Key: L L (magit2-toggle-margin) ::

  This command shows or hides the margin.

- Key: L l (magit2-cycle-margin-style) ::

  This command cycles the style used for the margin.

- Key: L d (magit2-toggle-margin-details) ::
...
- Key: C-c C-c (magit2-log-select-pick) ::

  Select the commit at point and act on it.  Call
  ~magit2-log-select-pick-function~ with the selected commit as
  argument.

- Key: C-c C-k (magit2-log-select-quit) ::

  Abort selecting a commit, don't act on any commit.

- User Option: magit2-log-select-margin ::
...
- Key: l r (magit2-reflog-current) ::

  Display the reflog of the current branch.

- Key: l O (magit2-reflog-other) ::

  Display the reflog of a branch or another ref.

- Key: l H (magit2-reflog-head) ::

  Display the ~HEAD~ reflog.

- User Option: magit2-reflog-margin ::
...
The command ~magit2-cherry~ shows cherries for a single branch, but the
...
- User Option: magit2-cherry-margin ::
...
but that obviously isn't enough.  The transient prefix command
~magit2-diff~, on ~d~, features several suffix commands, which show a
specific diff in a separate diff buffer.

Like other transient prefix commands, ~magit2-diff~ also features several
infix arguments that can be changed before invoking one of the suffix
...
buffer, depending on the value of ~magit2-prefix-use-buffer-arguments~
(see [[*Transient Arguments and Buffer Variables]]).
...
- Key: d d (magit2-diff-dwim) ::

  Show changes for the thing at point.

- Key: d r (magit2-diff-range) ::
...
- Key: d u (magit2-diff-unstaged) ::

  Show changes between the working tree and the index.

- Key: d p (magit2-diff-paths) ::
...
The transient prefix command ~magit2-diff-refresh~, on ~D~, can be used to
...
- Key: D (magit2-diff-refresh) ::

  This transient prefix command binds the following suffix commands
  along with the appropriate infix arguments and displays them in a
  temporary buffer until a suffix is invoked.

- Key: D g (magit2-diff-refresh) ::

  This suffix command sets the local diff arguments for the current
  buffer.

- Key: D s (magit2-diff-set-default-arguments) ::
...
- Key: D w (magit2-diff-save-default-arguments) ::
...
- Key: C-c C-d (magit2-diff-while-committing) ::
...
~magit2-diff-visit-file~ and related commands visit the appropriate
version of the file that the diff at point is about.  Likewise
~magit2-diff-visit-worktree-file~ and related commands visit the worktree
...
- Key: C-c C-t (magit2-diff-trace-definition) ::

  This command shows a log for the definition at point.

- User Option: magit2-log-trace-definition-function ::

  The function specified by this option is used by
  ~magit2-log-trace-definition~ to determine the function at point.  For
  major-modes that have special needs, you could set the local value
  using the mode's hook.

- Key: C-c C-e (magit2-diff-edit-hunk-commit) ::
  ...
  location using ~magit2-diff-visit-file~.  This actually visits a blob.
  When point is on a diff header, not within an individual hunk, then
  this visits the blob the first hunk is about.

  Then it invokes ~magit2-edit-line-commit~, which uses an interactive
...
- Key: j (magit2-jump-to-diffstat-or-diff) ::
...
- User Option: magit2-diff-refine-hunk ::
...
- User Option: magit2-diff-refine-ignore-whitespace ::

  Whether to ignore whitespace changes in word-granularity
  differences.

- User Option: magit2-diff-adjust-tab-width ::
  ...
  variable ~magit2-diff--tab-width-cache~.  Set that to nil to invalidate
  the cache.
...
- User Option: magit2-diff-paint-whitespace ::

  Specify where to highlight whitespace errors.

  See ~magit2-diff-highlight-trailing~,
  ~magit2-diff-highlight-indentation~.  The symbol ~t~ means in all
...
- User Option: magit2-diff-paint-whitespace-lines ::
...
- User Option: magit2-diff-highlight-trailing ::

  Whether to highlight whitespace at the end of a line in diffs.  Used
  only when ~magit2-diff-paint-whitespace~ is non-nil.

- User Option: magit2-diff-highlight-indentation ::
  ...
  if ~magit2-diff-paint-whitespace~ is also non-nil.
...
- User Option: magit2-diff-hide-trailing-cr-characters ::

  Whether to hide ^M characters at the end of a line in diffs.

- User Option: magit2-diff-highlight-hunk-region-functions ::

  This option specifies the functions used to highlight the
  hunk-internal region.

  ~magit2-diff-highlight-hunk-region-dim-outside~ overlays the outside of
  ...
  https://github.com/magit2/magit2/issues/2758 ff.
...
- User Option: magit2-diff-unmarked-lines-keep-foreground ::
  ...
  dimmed at all depends on ~magit2-diff-highlight-hunk-region-functions~.

- User Option: magit2-diff-extra-stat-arguments ::
...
- User Option: magit2-revision-insert-related-refs ::
...
- User Option: magit2-revision-show-gravatars ::
  ...
  If you have customized the option ~magit2-revision-headers-format~
...
- User Option: magit2-revision-use-hash-sections ::
...
~magit2-diff-toggle-file-filter~).

- User Option: magit2-revision-filter-files-on-follow ::
...
- Key: e (magit2-ediff-dwim) ::
  ...
  in which case the appropriate ~magit2-ediff-*~ command has to be used
...
- Key: E t (magit2-git-mergetool) ::

  This command does not actually use Ediff.  While it serves the same
  purpose as =magit2-ediff-resolve=, it uses =git mergetool --gui= to
  resolve conflicts.

  With a prefix argument this acts as a transient prefix command,
  allowing the user to select the mergetool and change some settings.

- Key: E s (magit2-ediff-stage) ::

  Stage and unstage changes to a file using Ediff, defaulting to the
  file at point.

- Key: E u (magit2-ediff-show-unstaged) ::

  Show unstaged changes to a file using Ediff.

- Key: E i (magit2-ediff-show-staged) ::

  Show staged changes to a file using Ediff.

- Key: E w (magit2-ediff-show-working-tree) ::

  Show changes in a file between ~HEAD~ and working tree using Ediff.

- Key: E c (magit2-ediff-show-commit) ::

  Show changes to a file introduced by a commit using Ediff.

- Key: E z (magit2-ediff-show-stash) ::

  Show changes to a file introduced by a stash using Ediff.

- User Option: magit2-ediff-dwim-show-on-hunks ::

  This option controls what command ~magit2-ediff-dwim~ calls when
  point is on uncommitted hunks.  When nil, always run
  ~magit2-ediff-stage~.  Otherwise, use ~magit2-ediff-show-staged~ and
  ~magit2-ediff-show-unstaged~ to show staged and unstaged changes,
  respectively.

- User Option: magit2-ediff-show-stash-with-index ::

  This option controls whether ~magit2-ediff-show-stash~ includes a
...
- User Option: magit2-ediff-quit-hook ::
...
- User Option: magit2-refs-pad-commit-counts ::
  ...
  similar to ~magit2-dimmed~.

  If this is non-nil, then spaces are placed on both sides of all
  commit counts.

- User Option: magit2-refs-show-remote-prefix ::
...
- User Option: magit2-refs-primary-column-width ::

  Width of the primary column in `magit2-refs-mode' buffers.  The
...
- User Option: magit2-refs-focus-column-width ::
...
- User Option: magit2-refs-margin ::
...
- User Option: magit2-refs-margin-for-tags ::
...
# new list
- User Option: magit2-refs-filter-alist ::
  ...
  displayed in ~magit2-refs-mode~ buffers.  If it is ~nil~, then all refs
  are displayed (subject to ~magit2-refs-sections-hook~).
...
- Key: RET (magit2-visit-ref) ::
  ...
  This command behaves just like ~magit2-show-commit~ as described above,
  except if point is on a reference in a ~magit2-refs-mode~ buffer, in
  which case the behavior may be different, but only if you have
  customized the option ~magit2-visit-ref-behavior~.

- User Option: magit2-visit-ref-behavior ::

  This option controls how ~magit2-visit-ref~ behaves in ~magit2-refs-mode~
  buffers.

  By default ~magit2-visit-ref~ behaves like ~magit2-show-commit~, in all
  buffers, including ~magit2-refs-mode~ buffers.  When the type of the
  section at point is ~commit~ then "RET" is bound to ~magit2-show-commit~,
  ...
  However "RET" used to behave differently in ~magit2-refs-mode~ buffers,
  ...
  might have to resort to ~M-x magit2-show-commit~ to get it back.

  ~magit2-visit-ref~ looks for these symbols in the order in which they
...
~magit2-refs-sections-hook~.  See [[*Section Hooks]] to learn about such hooks
...
- User Option: magit2-refs-sections-hook ::

  Hook run to insert sections into a references buffer.

- Function: magit2-insert-local-branches ::

  Insert sections showing all local branches.

- Function: magit2-insert-remote-branches ::

  Insert sections showing all remote-tracking branches.

- Function: magit2-insert-tags ::
...
- User Option: magit2-bisect-show-graph ::
...
- Command: magit2-find-file-other-window ::

  This command reads a filename and revision from the user and visits
  the respective blob in a buffer.  The buffer is displayed in another
  window.

- Command: magit2-find-file-other-frame ::
...
- Key: RET (magit2-diff-visit-file) ::
...
- User Option: magit2-diff-visit-previous-blob ::

  This option controls whether ~magit2-diff-visit-file~ may visit the
  previous blob.  When this is ~t~ (the default) and point is on a
  removed line in a diff for a committed change, then
  ~magit2-diff-visit-file~ visits the blob from the last revision which
  still had that line.

  Currently this is only supported for committed changes, for staged
  and unstaged changes ~magit2-diff-visit-file~ always visits the file in
  the working tree.

- Key: C-<return> (magit2-diff-visit-file-worktree) ::

  This command visits the worktree version of the appropriate file.
  The location of point inside the diff determines which file is being
  visited.  Unlike ~magit2-diff-visit-file~ it always visits the "real"
...
- Command: magit2-diff-visit-file-other-window ::
- Command: magit2-diff-visit-file-other-frame ::
- Command: magit2-diff-visit-worktree-file-other-window ::
- Command: magit2-diff-visit-worktree-file-other-frame ::

** Blaming

Also see [[man:git-blame]]

To start blaming invoke the ~magit2-file-dispatch~ transient prefix
...
- Key: C-c M-g B b ::
  ...
  ~magit2-find-file~), where REVISION is a parent of the revision that
  added the current line or chunk of lines.

- Key: C-c M-g r (magit2-blame-removal) ::
- Key: C-c M-g B r ::
  ...
  Like ~magit2-blame-addition~, this command can be used recursively.

- Key: C-c M-g f (magit2-blame-reverse) ::
- Key: C-c M-g B f ::
...
- Key: C-c M-g e (magit2-blame-echo) ::
- Key: C-c M-g B e ::

  This command is like ~magit2-blame-addition~ except that it doesn't
  turn on ~read-only-mode~ and that it initially uses the visualization
  style specified by option ~magit2-blame-echo-style~.
...
- Key: c (magit2-blame-cycle-style) ::
...
- User Option: magit2-blame-goto-chunk-hook ::
...
command ~magit2-clone~.  This command either act as a transient prefix
...
argument is used and on the value of ~magit2-clone-always-transient~.

- User Option: magit2-clone-always-transient ::

  This option controls whether the command ~magit2-clone~ always acts as
...
- Key: C > (magit2-clone-sparse) ::
  ...
  ~magit2-sparse-checkout~ transient (see [[*Sparse checkouts]]).

- Key: C b (magit2-clone-bare) ::

  This command creates a bare clone of an existing repository.
  The repository and the target directory are read from the user.

- Key: C m (magit2-clone-mirror) ::
...
- Key: C d (magit2-clone-shallow-since) ::
...
- Key: C e (magit2-clone-shallow-exclude) ::
...
- User Option: magit2-clone-set-remote-head ::
...
- User Option: magit2-clone-set-remote.pushDefault ::
...
- User Option: magit2-clone-default-directory ::
...
- User Option: magit2-clone-name-alist ::
  ...
  The format specified by option ~magit2-clone-url-format~ is used to
...
- User Option: magit2-clone-url-format ::
...
- Key: U (magit2-unstage-all) ::

  Remove all changes from the staging area.

- User Option: magit2-unstage-committed ::

  This option controls whether ~magit2-unstage~ "unstages" committed
  changes by reversing them in the index but not the working tree.
  The alternative is to raise an error.

- Key: M-x magit2-reverse-in-index ::
  ...
  command, but it is indirectly called when ~u~ (~magit2-unstage~) is
...
- Key: c e (magit2-commit-extend) ::
  ...
  ~magit2-commit-extend-override-date~ can be used to inverse the meaning
  of the prefix argument.

  Non-interactively respect the optional OVERRIDE-DATE argument and
  ignore the option.

- Key: c w (magit2-commit-reword) ::

  Reword the last commit, ignoring staged changes.  With a prefix
  argument keep the committer date, otherwise change it.  The option
  ~magit2-commit-reword-override-date~ can be used to inverse the meaning
  of the prefix argument.

  Non-interactively respect the optional OVERRIDE-DATE argument and
  ignore the option.

- Key: c f (magit2-commit-fixup) ::
...
- Key: c F (magit2-commit-instant-fixup) ::

  Create a fixup commit and instantly rebase.

- Key: c s (magit2-commit-squash) ::
...
- Key: c S (magit2-commit-instant-squash) ::

  Create a squash commit and instantly rebase.

- Key: c A (magit2-commit-augment) ::
...
- User Option: magit2-commit-diff-inhibit-same-window ::
...
~magit2-commit-diff~ from ~server-switch-hook~.
...
- Key: C-c C-w (magit2-pop-revision-stack) ::
  ...
  the ~magit2-revision-stack~ and inserts it into the current buffer
  according to ~magit2-pop-revision-stack-format~.  Revisions can be put
  on the stack using ~magit2-copy-section-value~ and
  ~magit2-copy-buffer-revision~.
...
- User Option: magit2-pop-revision-stack-format ::

  This option controls how the command ~magit2-pop-revision-stack~
...
- Function: git-commit-propertize-diff ::
  ...
  ~--verbose~ argument is used.  ~magit2-commit~ by default does not offer
...
- User Option: git-commit-setup-hook ::
  ...
  at all.  For certain commands such as ~magit2-rebase-continue~ this
  hook is never run because doing so would lead to a race condition.

  This hook is only run if ~magit2~ is available.

  Also see ~magit2-post-commit-hook~.
...
changed using the command ~magit2-branch-configure~, which is available
...
The transient prefix command ~magit2-branch~ is used to create and
...
- User Option: magit2-branch-direct-configure ::

  This option controls whether the transient command ~magit2-branch~ can
  ...
  command ~magit2-branch-configure~ has to be used instead to view and
  change branch related variables.

- Key: b C (magit2-branch-configure) ::
- Key: f C ::
- Key: F C ::
- Key: P C ::
  ...
  a suffix of ~magit2-branch~ and on the ~magit2-branch-direct-configure~
  option.  If ~magit2-branch~ already displays the variables for the
...
- Key: b n (magit2-branch-create) ::
  ...
  Also see option ~magit2-branch-prefer-remote-upstream~.

- Key: b c (magit2-branch-and-checkout) ::

  This command creates a new branch like ~magit2-branch-create~, but then
  also checks it out.

  Also see option ~magit2-branch-prefer-remote-upstream~.

- Key: b l (magit2-branch-checkout) ::
  ...
  of ~magit2-branch-adjust-remote-upstream-alist~.

- Key: b s (magit2-branch-spinoff) ::
  ...
  ~magit2-branch-prefer-remote-upstream~ (which see), then the current
...
- Key: b S (magit2-branch-spinout) ::

  This command behaves like ~magit2-branch-spinoff~, except that it does
  not change the current branch.  If there are any uncommitted changes,
  then it behaves exactly like ~magit2-branch-spinoff~.

- Key: b x (magit2-branch-reset) ::
...
- User Option: magit2-branch-read-upstream-first ::
...
- User Option: magit2-branch-prefer-remote-upstream ::
...
- User Option: magit2-branch-adjust-remote-upstream-alist ::
...
- Command: magit2-branch-orphan ::

  This command creates and checks out a new orphan branch with
  contents from a given revision.

- Command: magit2-branch-or-checkout ::

  This command is a hybrid between ~magit2-checkout~ and
  ~magit2-branch-and-checkout~ and is intended as a replacement for the
  former in ~magit2-branch~.
  ...
  checks that out, just like ~magit2-checkout~ would.
  ...
  branch.  This is similar to what ~magit2-branch-and-checkout~ does.

  To use this command instead of ~magit2-checkout~ add this to your init
  file:

  #+BEGIN_SRC emacs-lisp
    (transient-replace-suffix 'magit2-branch 'magit2-checkout
      '("b" "dwim" magit2-branch-or-checkout))
  #+END_SRC

*** Branch Git Variables

These variables can be set from the transient prefix command
~magit2-branch-configure~.  By default they can also be set from
~magit2-branch~.  See [[*Branch Commands]].
...
- User Option: magit2-prefer-remote-upstream ::
  ...
  This affects all commands that use ~magit2-read-upstream-branch~ or
  ~magit2-read-starting-point~, which includes all commands that change
  the upstream and many which create new branches.

*** Auxiliary Branch Commands

These commands are not available from the transient ~magit2-branch~ by
default.

- Command: magit2-branch-shelve ::
...
- Command: magit2-branch-unshelve ::

  This command unshelves a branch that was previously shelved using
  ~magit2-branch-shelve~.  This is done by deleting the reference
...
- Key: m a (magit2-merge-absorb) ::
  ...
  Finally, if ~magit2-branch-pull-request~ was used to create the merged
  branch, then the respective remote branch is also removed.

- Key: m i (magit2-merge-into) ::
...
Additionally ~k~ (~magit2-discard~) on a hunk with unresolved conflicts
...
- Key: r p (magit2-rebase-onto-pushremote) ::
...
- Key: r u (magit2-rebase-onto-upstream) ::
...
- Key: r r (magit2-rebase-continue) ::
...
- Key: A A (magit2-sequence-continue) ::

  Resume the current cherry-pick or revert sequence.

- Key: A s (magit2-sequence-skip) ::

  Skip the stopped at commit during a cherry-pick or revert sequence.

- Key: A a (magit2-sequence-abort) ::
...
- Key: V V (magit2-revert-and-commit) ::
...
- Key: V v (magit2-revert-no-commit) ::
...
- Key: V A (magit2-sequence-continue) ::

  Resume the current cherry-pick or revert sequence.

- Key: V s (magit2-sequence-skip) ::

  Skip the stopped at commit during a cherry-pick or revert sequence.

- Key: V a (magit2-sequence-abort) ::
...
- Key: X m (magit2-reset-mixed) ::

  Reset the ~HEAD~ and index to some commit read from the user and
  defaulting to the commit at point.  The working tree is kept as-is.

- Key: X s (magit2-reset-soft) ::
...
- Key: z x (magit2-stash-keep-index) ::
...
The transient prefix command ~magit2-remote~ is used to add remotes and
...
- User Option: magit2-remote-direct-configure ::

  This option controls whether remote-related Git variables are
  accessible directly from the transient ~magit2-remote~.

  If ~t~ (the default) and a local branch is checked out, then
  ~magit2-remote~ features the variables for the upstream remote of that
  branch, or if ~HEAD~ is detached, for ~origin~, provided that exists.

  If ~nil~, then ~magit2-remote-configure~ has to be used to do so.

- Key: M C (magit2-remote-configure) ::
  ...
  Without a prefix argument this depends on whether it was invoked as
  a suffix of ~magit2-remote~ and on the ~magit2-remote-direct-configure~
  option.  If ~magit2-remote~ already displays the variables for the
...
- User Option: magit2-remote-add-set-remote.pushDefault ::
...
These variables can be set from the transient prefix command
~magit2-remote-configure~.  By default they can also be set from
~magit2-remote~.  See [[*Remote Commands]].
...
- Key: f p (magit2-fetch-from-pushremote) ::
...
- Key: f u (magit2-fetch-from-upstream) ::
...
- User Option: magit2-pull-or-fetch ::
  ...
  of the above suffix commands to the ~magit2-pull~ transient.

  If you do that, then you might also want to change the key binding
  for these prefix commands, e.g.:

  #+BEGIN_SRC emacs-lisp
    (setq magit2-pull-or-fetch t)
    (define-key magit2-mode-map "f" 'magit2-pull) ; was magit2-fetch
    (define-key magit2-mode-map "F" nil)         ; was magit2-pull
  #+END_SRC
...
- Key: F p (magit2-pull-from-pushremote) ::
...
- Key: F u (magit2-pull-from-upstream) ::
...
- Key: P p (magit2-push-current-to-pushremote) ::
...
- Key: P u (magit2-push-current-to-upstream) ::
...
- Command: magit2-push-implicitly args ::
...
- Command: magit2-push-to-remote remote args ::
...
- Key: w w (magit2-am-apply-patches) ::
...
- Key: w m (magit2-am-apply-maildir) ::
...
- Key: t r (magit2-tag-release) ::
  ...
  ~v1.2.3-custom.1~), you can set the ~magit2-release-tag-regexp~ and
  ~magit2-tag-version-regexp-alist~ variables.
...
- Key: T c (magit2-notes-merge-commit) ::

  Commit the current notes ref merge, after manually resolving
  conflicts.

- Key: T a (magit2-notes-merge-abort) ::

  Abort the current notes ref merge.

The following variables control what notes reference ~magit2-notes-*~,
...
The command ~magit2-list-submodules~ displays a list of the current
...
the super-repository by adding ~magit2-insert-modules~ to the hook
~magit2-status-sections-hook~ as described in [[*Status Module Sections]].

- Command: magit2-list-submodules ::
...
- User Option: magit2-submodule-list-columns ::

  This option controls what columns are displayed by the command
  ~magit2-list-submodules~ and how they are displayed.
...
- Key: o r (magit2-submodule-register) ::
...
- Key: Z (magit2-worktree) ::

  This transient prefix command binds the following suffix commands
  and displays them in a temporary buffer until a suffix is invoked.

- Key: Z b (magit2-worktree-checkout) ::

  Checkout BRANCH in a new worktree at PATH.

- Key: Z c (magit2-worktree-branch) ::

  Create a new BRANCH and check it out in a new worktree at PATH.

- Key: Z m (magit2-worktree-move) ::

  Move an existing worktree to a new PATH.

- Key: Z k (magit2-worktree-delete) ::

  Delete a worktree, defaulting to the worktree at point.
  The primary worktree cannot be deleted.

- Key: Z g (magit2-worktree-status) ::
...
- Key: > (magit2-sparse-checkout) ::

  This transient prefix command binds the following suffix commands
  and displays them in a temporary buffer until a suffix is invoked.

- Key: > e (magit2-sparse-checkout-enable) ::

  This command initializes a sparse checkout that includes only the
  files in the top-level directory.

  Note that ~magit2-sparse-checkout-set~ and
  ~magit2-sparse-checkout-add~ automatically initialize a sparse
  checkout if necessary.  However, you may want to call
  ~magit2-sparse-checkout-enable~ explicitly to re-initialize a sparse
  checkout after calling ~magit2-sparse-checkout-disable~, to pass
  additional arguments to ~git sparse-checkout init~, or to execute
  the initialization asynchronously.

- Key: > s (magit2-sparse-checkout-set) ::
...
- Key: > a (magit2-sparse-checkout-add) ::

  This command is like ~magit2-sparse-checkout-set~, but instead adds
  the specified list of directories to the set of directories that is
  already included in the sparse checkout.

- Key: > r (magit2-sparse-checkout-reapply) ::
...
- Command: magit2-switch-to-repository-buffer ::
+ Command: magit2-switch-to-repository-buffer-other-window ::
+ Command: magit2-switch-to-repository-buffer-other-frame ::
+ Command: magit2-display-repository-buffer ::
  ...
  The last variant uses ~magit2-display-buffer~ to do so and thus
  respects ~magit2-display-buffer-function~.

These are some of the commands that can be used in all buffers whose
major-modes derive from ~magit2-mode~.  There are other common commands
beside the ones below, but these didn't fit well anywhere else.

- Key: C-w (magit2-copy-section-value) ::
...
- Key: M-w (magit2-copy-buffer-revision) ::

  This command saves the revision being displayed in the current buffer
  to the ~kill-ring~ and also pushes it to the ~magit2-revision-stack~.  It
  is mainly intended for use in ~magit2-revision-mode~ buffers, the only
...
- User Option: magit2-wip-mode ::
  ...
  ~magit2-wip-*~ modes, which can also be used individually, if you want
...
~magit2-wip-log~ and ~magit2-wip-log-current~.  You should use ~--graph~ when
using these commands.

- Command: magit2-wip-log ::
  ...
  value of ~magit2-wip-merge-branch~ is ~nil~.

- Command: magit2-wip-log-current ::
...
Note that even if you enable ~magit2-wip-mode~ this won't give you
perfect protection.  The most likely scenario for losing changes
despite the use of ~magit2-wip-mode~ is making a change outside Emacs and
...
- User Option: magit2-wip-namespace ::
...
- User Option: magit2-wip-mode-lighter ::

  Mode-line lighter for ~magit2-wip--mode~.

*** Wip Graph

- User Option: magit2-wip-merge-branch ::
...
When ~magit2-wip-merge-branch~ is ~t~, then the history looks like this:
...
When ~magit2-wip-merge-branch~ is ~nil~, then creating a commit on the real
...
It is recommended that you use the mode ~magit2-wip-mode~ (which see) and
...
- User Option: magit2-wip-after-save-mode ::
...
- User Option: magit2-wip-after-apply-mode ::
...
- User Option: magit2-wip-before-change-mode ::
...
- User Option: magit2-wip-initial-backup-mode ::
...
- User Option: magit2-wip-after-save-local-mode-lighter ::

  Mode-line lighter for ~magit2-wip-after-save-local-mode~.

- User Option: magit2-wip-after-apply-mode-lighter ::

  Mode-line lighter for ~magit2-wip-after-apply-mode~.

- User Option: magit2-wip-before-change-mode-lighter ::

  Mode-line lighter for ~magit2-wip-before-change-mode~.

- User Option: magit2-wip-initial-backup-mode-lighter ::

  Mode-line lighter for ~magit2-wip-initial-backup-mode~.

** Commands for Buffers Visiting Files

Magit defines a few global key bindings unless the user sets
~magit2-define-global-key-bindings~ to ~nil~.  This includes binding ~C-c
M-g~ to ~magit2-file-dispatch~.  ~C-c g~ would be a much better binding
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c g") 'magit2-file-dispatch)
#+end_src

The key bindings shown below assume that you have not improved the
binding for ~magit2-file-dispatch~.

- Key: C-c M-g (magit2-file-dispatch) ::

  This transient prefix command binds the following suffix commands
  and displays them in a temporary buffer until a suffix is invoked.

  When invoked in a buffer that does not visit a file, then it falls
  back to regular ~magit2-dispatch~.

- Key: C-c M-g s (magit2-stage-file) ::

  Stage all changes to the file being visited in the current buffer.

- Key: C-c M-g u (magit2-unstage-file) ::

  Unstage all changes to the file being visited in the current buffer.

- Key: C-c M-g c (magit2-commit) ::
...
- Key: C-c M-g d (magit2-diff-buffer-file) ::

  This command shows the diff for the file of blob that the current
  buffer visits.

- User Option: magit2-diff-buffer-file-locked ::

  This option controls whether ~magit2-diff-buffer-file~ uses a dedicated
  buffer.  See [[*Modes and Buffers]].

- Key: C-c M-g L (magit2-log) ::
...
- Key: C-c M-g l (magit2-log-buffer-file) ::
...
- Key: C-c M-g t (magit2-log-trace-definition) ::

  This command shows the log for the definition at point.

- User Option: magit2-log-buffer-file-locked ::

  This option controls whether ~magit2-log-buffer-file~ uses a dedicated
  buffer.  See [[*Modes and Buffers]].
...
- Key: C-c M-g p (magit2-blob-previous) ::
...
The ~magit2-blob-mode~ enables certain Magit features in blob-visiting
buffers.  Such buffers can be created using ~magit2-find-file~ and some
...
- Key: p (magit2-blob-previous) ::

  Visit the previous blob which modified the current file.

- Key: n (magit2-blob-next) ::

  Visit the next blob which modified the current file.

- Key: q (magit2-kill-this-buffer) ::
...
- Certain settings like ~fetch.prune=true~ are respected by Magit
  ...
  In this case the ~--prune~ argument in ~magit2-fetch~ might be active or
...
- ~/path/to/huge/repo/.dir-locals.el~
  ...
  #+BEGIN_SRC emacs-lisp
    ((nil . ((magit2-refresh-buffers . nil))))
  #+END_SRC
...
- ~/path/to/tag/invested/repo/.dir-locals.el~
  ...
  #+BEGIN_SRC emacs-lisp
    ((magit2-status-mode
      . ((eval . (magit2-disable-section-inserter 'magit2-insert-tags-header)))))
  #+END_SRC

# new list
- Function: magit2-disable-section-inserter fn ::
...
disabled by adding a symbol to ~magit2-no-confirm~ (see [[*Completion and
...
can recover it from there.  Option ~magit2-delete-by-moving-to-trash~
controls whether the system trash is used, which is the case by default.
Nevertheless, ~trash~ isn't a member of ~magit2-no-confirm~ - you
...
#+begin_src emacs-lisp
  (setq magit2-refresh-status-buffer nil)
#+end_src

You should also check whether any third-party packages have added
anything to ~magit2-refresh-buffer-hook~, ~magit2-status-refresh-hook~,
~magit2-pre-refresh-hook~, and ~magit2-post-refresh-hook~.  If so, then
check whether those additions impact performance significantly.

Magit can be told to refresh buffers verbosely using ~M-x
magit2-toggle-verbose-refresh~.  Enabling this helps figuring out which
...
~magit2-insert-tags-headers~, as explained at the mentioned node.
...
~magit2-diff-highlight-indentation~, ~magit2-diff-highlight-trailing~,
~magit2-diff-paint-whitespace~, ~magit2-diff-highlight-hunk-body~, and
~magit2-diff-refine-hunk~.
...
a commit using ~magit2-revision-mode~ takes considerably more time than
looking at the same commit in ~magit2-diff-mode~, then consider setting
~magit2-revision-insert-related-refs~ to ~nil~.
...
#+begin_src emacs-lisp
  (remove-hook 'magit2-refs-sections-hook 'magit2-insert-tags)
#+end_src
...
#+begin_src emacs-lisp
  (remove-hook 'server-switch-hook 'magit2-commit-diff)
#+end_src
...
- User Option: magit2-define-global-key-bindings ::
...
https://github.com/magit2/magit2/wiki/Plugin-Dispatch-Key-Registry.

** Calling Git

Magit provides many specialized functions for calling Git.  All of
these functions are defined in either ~magit2-git.el~ or ~magit2-process.el~
and have one of the prefixes ~magit2-run-~, ~magit2-call-~, ~magit2-start-~,
or ~magit2-git-~ (which is also used for other things).
...
- Function: magit2-process-git destination &rest args ::
...
- Function: magit2-process-file process &optional infile buffer display &rest args ::
...
- User Option: magit2-git-debug ::

  Whether to report errors that occur when using ~magit2-git-insert~,
  ~magit2-git-string~, ~magit2-git-lines~, or ~magit2-git-items~.  This does
...
- Function: magit2-git-str &rest args ::

  This is a variant of ~magit2-git-string~ that ignores the option
  ~magit2-git-debug~.  It is mainly intended to be used while handling
...
- Function: magit2-call-git &rest args ::

  Calls git synchronously with ARGS.

- Function: magit2-call-process program &rest args ::

  Calls PROGRAM synchronously with ARGS.

- Function: magit2-run-git &rest args ::

  Calls git synchronously with ARGS and then refreshes.

- Function: magit2-run-git-with-input &rest args ::
...
- Function: magit2-run-git-async &rest args ::
  ...
  repository are reverted if ~magit2-revert-buffers~ is non-nil.

- Function: magit2-run-git-with-editor &rest args ::
...
- Function: magit2-start-git input &rest args ::
  ...
  Option ~magit2-git-executable~ specifies the Git executable and option
  ~magit2-git-global-arguments~ specifies constant arguments.  The
...
- Function: magit2-start-process &rest args ::
  ...
  use the sentinel ~magit2-process-sentinel~ and the filter
  ~magit2-process-filter~.  Information required by these functions is
  ...
  buffer that was current when ~magit2-start-process~ was called (if it
  ...
  in the current repository are reverted if ~magit2-revert-buffers~ is
  non-nil.

- Variable: magit2-this-process ::

  The child process which is about to start.  This can be used to
  change the filter and sentinel.

- Variable: magit2-process-raise-error ::

  When this is non-nil, then ~magit2-process-sentinel~ raises an error if
  git exits with a non-zero exit status.  For debugging purposes.

** Section Plumbing
*** Creating Sections

- Macro: magit2-insert-section &rest args ::
  ...
  a variable ~magit2-TYPE-section-map~ exists, then use that as the
  ...
  ~magit2-section-set-visibility-hook~.  When a section is recreated
  ...
  ~magit2-cancel-section~ can be used to abort and remove all traces of
...
- Function: magit2-insert-heading &rest args ::
  ...
  ~magit2-section-heading~ face for all inserted text.
...
- Function: magit2-cancel-section ::

  Cancel the section currently being inserted.  This exits the
  innermost call to ~magit2-insert-section~ and removes all traces of
  what has already happened inside that call.

- Function: magit2-define-section-jumper sym title &optional value ::
...
- Function: magit2-current-section ::

  Return the section at point.

- Function: magit2-region-sections &optional condition multiple ::
...
- Function: magit2-region-values &optional condition multiple ::
...
- Key: M-x magit2-describe-section-briefly ::

  Show information about the section at point.  This command is
  intended for debugging purposes.

- Function: magit2-section-ident section ::

  Return an unique identifier for SECTION.  The return value has the
  form ~((TYPE . VALUE)...)~.

- Function: magit2-get-section ident &optional root ::

  Return the section identified by IDENT.  IDENT has to be a list as
  returned by ~magit2-section-ident~.

- Function: magit2-section-match condition &optional section ::
...
- Function: magit2-section-value-if condition &optional section ::
...
- Function: magit2-section-case &rest clauses ::
...
- Variable: magit2-root-section ::
  ...
  ~magit2-insert-section~ and you should never modify it.

For diff related sections a few additional tools exist.

- Function: magit2-diff-type &optional section ::
  ...
  corresponding ~magit2-section~ struct) but takes additional information
  ...
  ~magit2-diff-mode~ SECTION is ignored and the type is determined using
  other means.  In ~magit2-revision-mode~ buffers the type is always
  ~committed~.

- Function: magit2-diff-scope &optional section strict ::
  ...
  confuse this with the diff "type", as returned by ~magit2-diff-type~.
...
displayed in an existing buffer do so by calling ~magit2-mode-setup~.
Among other things, that function sets the buffer local values of
~default-directory~ (to the top-level of the repository),
~magit2-refresh-function~, and ~magit2-refresh-args~.

Buffers are refreshed by calling the function that is the local value
of ~magit2-refresh-function~ (a function named ~magit2-*-refresh-buffer~,
where ~*~ may be something like ~diff~) with the value of
~magit2-refresh-args~ as arguments.

- Macro: magit2-mode-setup buffer switch-func mode refresh-func &optional refresh-args ::
  ...
  ~magit2-mode-display-buffer~ with BUFFER, MODE and SWITCH-FUNC as
  arguments.  Then it sets the local value of ~magit2-refresh-function~
  to REFRESH-FUNC and that of ~magit2-refresh-args~ to REFRESH-ARGS.
...
- Function: magit2-mode-display-buffer buffer mode &optional switch-function ::
  ...
  can later be restored by ~magit2-mode-bury-buffer~.
  ...
  derives from ~magit2-mode~.  Otherwise ~switch-to-buffer~ is used.

- Variable: magit2-refresh-function ::

  The value of this buffer-local variable is the function used to
  refresh the current buffer.  It is called with ~magit2-refresh-args~ as
  arguments.

- Variable: magit2-refresh-args ::

  The list of arguments used by ~magit2-refresh-function~ to refresh the
  current buffer.  ~magit2-refresh-function~ is called with these
  arguments.

  The value is usually set using ~magit2-mode-setup~, but in some cases
  it's also useful to provide commands that can change the value.  For
  example, the ~magit2-diff-refresh~ transient can be used to change any
  of the arguments used to display the diff, without having to specify
  again which differences should be shown, but ~magit2-diff-more-context~,
  ~magit2-diff-less-context~ and ~magit2-diff-default-context~ change just
  the ~-U<N>~ argument.  In both case this is done by changing the value
  of this variable and then calling this ~magit2-refresh-function~.
...
special care to adjust ~magit2-branch-current~ accordingly.  By default
it looks mostly like ~magit2-branch-local~, but with a box (by default
...
enough yet to make it valid or because she wants to use a non-magit2
...
"highlight" in their names.  For most sections, ~magit2-section-highlight~
...
between hunks.  That face ~magit2-diff-hunk-heading~, should be different
from both ~magit2-diff-hunk-heading-highlight~ and
~magit2-section-highlight~, as well as from ~magit2-diff-context~ and
~magit2-diff-context-highlight~.  By default we do that by changing the
...
instead use one of the faces ~magit2-diff-file-heading-selection~ or
~magit2-diff-hunk-heading-selection~.  These faces have to be different
...
~magit2-diff-refine-hunk~ to ~all~.  You might personally prefer to only
...
(Also turn on ~magit2-diff-highlight-indentation~,
~magit2-diff-highlight-trailing~, and ~magit2-diff-paint-whitespace~; and
insert some whitespace errors into the code you use for testing.)

For added lines you have to adjust three faces:
~magit2-diff-added~, ~magit2-diff-added-highlight~, and
...
hunk heading is shown using ~magit2-diff-hunk-heading-selection~, and so
...
https://github.com/magit2/magit2/wiki/FAQ.
...
Also see https://magit2.vc/assets/videos/magic.mp4.
...
isn't available, ~M-x magit2-process-buffer~).  This will show a buffer
...
that approach, then set the value of ~magit2-view-git-manual-method~ to
one of the supported packages ~man~ or ~woman~, e.g.:

#+begin_src emacs-lisp
  (setq magit2-view-git-manual-method 'man)
#+end_src
...
Please see [[*Branching]] and http://emacsair.me/2016/01/18/magit2-2.4

*** Can Magit be used as ~ediff-version-control-package~?

No, it cannot.  For that to work the functions ~ediff-magit2-internal~
and ~ediff-magit2-merge-internal~ would have to be implemented, and they
...
~ediff-magit2-internal~ we provide ~magit2-ediff-compare~, which handles
...
~ediff-magit2-merge-internal~.  Instead we provide the more useful
command ~magit2-ediff-resolve~ which only shows yet-to-be resolved
conflicts.
...
https://github.com/magit2/magit2/wiki/Pushing-with-Magit-from-Windows.
...
#+begin_src emacs-lisp
  (load "/path/to/magit2-autoloads.el")
#+end_src
...
instead use ~magit2-diff-visit-file-hook~ to reveal the text, possibly
...
https://github.com/magit2/magit2/issues/3624.
...
https://magit2.vc/manual/transient/Saving-Values.html#Saving-Values for
more details.
...
- Key: M-x magit2-emacs-Q-command ::
  ...
  This shell command starts ~emacs~ with only ~magit2~ and its
...
- Key: M-x magit2-toggle-verbose-refresh ::
...
#  LocalWords:  logfile magit2 maildir manpage manpages minibuffer multi mv
...
# Local Variables:
# eval: (require 'magit2-base nil t)
