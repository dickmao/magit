;;; magit2-base.el --- early birds   -*- lexical-binding: t; coding: utf-8 -*-
...
(defconst magit2--minimal-git "2.2.0")
(defconst magit2--minimal-emacs "25.1")
...
(defvar magit2-wip-before-change-mode)

;;; Options

(defcustom magit2-completing-read-function 'magit2-builtin-completing-read
  "Function to be called when requesting input from the user.

If you have enabled `ivy-mode' or `helm-mode', then you don't
have to customize this option; `magit2-builtin-completing-read'
will work just fine.  However, if you use Ido completion, then
you do have to use `magit2-ido-completing-read', because Ido is
...
alphabetical order, depending on your version of Ivy."
  ...
  :type '(radio (function-item magit2-builtin-completing-read)
                (function-item magit2-ido-completing-read)
                (function-item ivy-completing-read)
                (function-item helm--completing-read-default)
                (function :tag "Other function")))

(defcustom magit2-dwim-selection
  '((magit2-stash-apply        nil t)
    (magit2-stash-branch       nil t)
    (magit2-stash-branch-here  nil t)
    (magit2-stash-format-patch nil t)
    (magit2-stash-drop         nil ask)
    (magit2-stash-pop          nil ask)
...
- COMMAND is the command that should not prompt for a choice.
  ...
  `magit2-completing-read' or a utility function which in turn uses
  that function.

- If the command uses `magit2-completing-read' multiple times, then
  ...
  the PROMPT argument passed to `magit2-completing-read'.

- DEFAULT specifies how to use the default.  If it is t, then
  the DEFAULT argument passed to `magit2-completing-read' is used
...
(defconst magit2--confirm-actions
  '((const discard)
    (const reverse)
...
(defcustom magit2-no-confirm '(set-and-push)
...
Files:
  ...
  Also see option `magit2-delete-by-moving-to-trash'.
...
References:
  ...
  remote, then `magit2-branch-delete' offers to delete the remote
...
Edit published history:
  ...
  branches listed in `magit2-published-branches'.
  ...
  `edit-published' Affects the commands `magit2-edit-line-commit'
  and `magit2-diff-edit-hunk-commit'.  These two commands make
  ...
  or set `magit2-published-branches' to nil.
...
Global settings:
  ...
  When `magit2-wip-before-change-mode' is enabled then these actions
  ...
  :type `(choice (const :tag "Always require confirmation" nil)
                 ...
                 (set   :tag "Require confirmation except for"
                        ...
                        ;; omitted.
                        ,@magit2--confirm-actions)))

(defcustom magit2-slow-confirm '(drop-stashes)
...
for `magit2-no-confirm' (which see)."
...
(defcustom magit2-no-message nil
  "A list of messages Magit should not display.

Magit displays most echo area messages using `message', but a few
are displayed using `magit2-message' instead, which takes the same
arguments as the former, FORMAT-STRING and ARGS.  `magit2-message'
...
maintainers to start using `magit2-message' instead of `message'
in that case.  We are not proactively replacing all uses of
`message' with `magit2-message', just in case someone *might* find
some of these messages useless.

Messages which can currently be suppressed using this option are:
* \"Turning on magit2-auto-revert-mode...\""
  :package-version '(magit2 . "2.8.0")
  :group 'magit2-miscellaneous
  :type '(repeat string))

(defcustom magit2-ellipsis (if (char-displayable-p ?…) "…" "...")
  "String used to abbreviate text in process buffers.

Currently this is only used to elide `magit2-git-global-arguments'
...
- Whether collapsed sections are indicated using ellipsis is
  controlled by `magit2-section-visibility-indicator'."
  :package-version '(magit2 . "3.0.0")
  :group 'magit2-miscellaneous
  :type 'string)

(defcustom magit2-update-other-window-delay 0.2
...
(defcustom magit2-view-git-manual-method 'info
...
(defclass magit2-commit-section (magit2-section)
  ())

(defclass magit2-file-section (magit2-section)
  ((keymap :initform 'magit2-file-section-map)
   (source :initform nil)
   (header :initform nil)))

(defclass magit2-module-section (magit2-file-section)
  ((keymap :initform 'magit2-hunk-section-map)
   (range  :initform nil)))

(defclass magit2-hunk-section (magit2-section)
  ((keymap      :initform 'magit2-hunk-section-map)
...
(setf (alist-get 'commit magit2--section-type-alist) 'magit2-commit-section)
(setf (alist-get 'file   magit2--section-type-alist) 'magit2-file-section)
(setf (alist-get 'module magit2--section-type-alist) 'magit2-module-section)
(setf (alist-get 'hunk   magit2--section-type-alist) 'magit2-hunk-section)
...
(defvar magit2-completing-read--silent-default nil)

(defun magit2-completing-read (prompt collection &optional
...
`magit2-dwim-selection'.  See the documentation of the latter for
more information.

If it does use the default without the user even having to
confirm that, then `magit2-completing-read--silent-default' is set
to t, otherwise nil.
...
- FALLBACK specifies a secondary default that is only used if
  ...
  subject to `magit2-dwim-selection' — if DEF is nil but FALLBACK
...
- PROMPT is modified to end with \" (default DEF|FALLBACK): \"
  ...
  `magit2-completing-read-function' is set to its default value of
  `magit2-builtin-completing-read'."
  (setq magit2-completing-read--silent-default nil)
  (if-let ((dwim (and def
                      (nth 2 (-first (pcase-lambda (`(,cmd ,re ,_))
                                       (and (eq this-command cmd)
                                            (or (not re)
                                                (string-match-p re prompt))))
                                     magit2-dwim-selection)))))
      (if (eq dwim 'ask)
          (if (y-or-n-p (format "%s %s? " prompt def))
              def
            (user-error "Abort"))
        (setq magit2-completing-read--silent-default t)
        def)
    (unless def
      (setq def fallback))
    (let ((command this-command)
          (reply (funcall magit2-completing-read-function
                          (concat prompt ": ")
...
(defun magit2--completion-table (collection)
  (lambda (string pred action)
    (if (eq action 'metadata)
        '(metadata (display-sort-function . identity))
      (complete-with-action action collection string pred))))

(defun magit2-builtin-completing-read
  ...
  (unless (or (bound-and-true-p helm-mode)
    ...
    (setq prompt (magit2-prompt-with-default prompt def)))
  (unless (or (bound-and-true-p helm-mode)
              (bound-and-true-p ivy-mode))
    (setq choices (magit2--completion-table choices)))
  (cl-letf (((symbol-function 'completion-pcm--all-completions)))
    (when (< emacs-major-version 26)
      (fset 'completion-pcm--all-completions
            'magit2-completion-pcm--all-completions))
    (let ((ivy-sort-functions-alist nil))
      (completing-read prompt choices
                       predicate require-match
                       initial-input hist def))))

(defun magit2-completing-read-multiple
...
into a list."
  (declare (obsolete magit2-completing-read-multiple* "Magit 3.1.0"))
  (let* ((crm-separator (or sep crm-default-separator))
         (crm-completion-table (magit2--completion-table choices))
...
third-party completion frameworks."
  (cl-letf*
      (;; To implement NO-SPLIT we have to manipulate the respective
       ...
       (table (magit2--completion-table table))
...
(defun magit2-ido-completing-read
    (prompt choices &optional predicate require-match initial-input hist def)
...
same name."
  (if (and (require 'ido-completing-read+ nil t)
    ...
    (display-warning 'magit2 "ido-completing-read+ is not installed
...
back to built-in `completing-read' for now." :error)
    (magit2-builtin-completing-read prompt choices predicate require-match
                                   initial-input hist def)))

(defun magit2-prompt-with-default (prompt def)
  (if (and def (> (length prompt) 2)
           (string-equal ": " (substring prompt -2)))
      (format "%s (default %s): " (substring prompt 0 -2) def)
    prompt))

(defvar magit2-minibuffer-local-ns-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map minibuffer-local-map)
    (define-key map "\s" 'magit2-whitespace-disallowed)
    (define-key map "\t" 'magit2-whitespace-disallowed)
    map))

(defun magit2-whitespace-disallowed ()
...
(defun magit2-read-string (prompt &optional initial-input history default-value
...
* an invalid DEFAULT-VALUE is silently ignored."
  ...
  (let* ((minibuffer-completion-table nil)
         (val (read-from-minibuffer
               (magit2-prompt-with-default (concat prompt ": ") default-value)
               initial-input (and no-whitespace magit2-minibuffer-local-ns-map)
...
(defun magit2-read-string-ns (prompt &optional initial-input history
                                    default-value inherit-input-method)
  "Call `magit2-read-string' with non-nil NO-WHITESPACE."
  (magit2-read-string prompt initial-input history default-value
                     inherit-input-method t))

(defmacro magit2-read-char-case (prompt verbose &rest clauses)
  (declare (indent 2)
...
(defun magit2-y-or-n-p (prompt &optional action)
  "Ask user a \"y or n\" or a \"yes or no\" question using PROMPT.
Which kind of question is used depends on whether
ACTION is a member of option `magit2-slow-confirm'."
  (if (or (eq magit2-slow-confirm t)
          (and action (member action magit2-slow-confirm)))
      (yes-or-no-p prompt)
    (y-or-n-p prompt)))

(defvar magit2--no-confirm-alist
  '((safe-with-wip magit2-wip-before-change-mode
                   discard reverse stage-all-changes unstage-all-changes)))

(cl-defun magit2-confirm (action &optional prompt prompt-n noabort
                                (items nil sitems))
  (declare (indent defun))
  (setq prompt-n (format (concat (or prompt-n prompt) "? ") (length items)))
  (setq prompt   (format (concat (or prompt (magit2-confirm-make-prompt action))
                                 "? ")
                         (car items)))
  (or (cond ((and (not (eq action t))
                  (or (eq magit2-no-confirm t)
                      (memq action magit2-no-confirm)
                      (cl-member-if (pcase-lambda (`(,key ,var . ,sub))
                                      (and (memq key magit2-no-confirm)
                                           (memq action sub)
                                           (or (not var)
                                               (and (boundp var)
                                                    (symbol-value var)))))
                                    magit2--no-confirm-alist)))
             (or (not sitems) items))
            ((not sitems)
             (magit2-y-or-n-p prompt action))
            ((= (length items) 1)
             (and (magit2-y-or-n-p prompt action) items))
            ((> (length items) 1)
             (and (magit2-y-or-n-p (concat (mapconcat #'identity items "\n")
                                          "\n\n" prompt-n)
                                  action)
                  items)))
      (if noabort nil (user-error "Abort"))))

(defun magit2-confirm-files (action files &optional prompt)
  (when files
    (unless prompt
      (setq prompt (magit2-confirm-make-prompt action)))
    (magit2-confirm action
      (concat prompt " %s")
      (concat prompt " %i files")
      nil files)))

(defun magit2-confirm-make-prompt (action)
  (let ((prompt (symbol-name action)))
    (replace-regexp-in-string
     "-" " " (concat (upcase (substring prompt 0 1)) (substring prompt 1)))))

(defun magit2-read-number-string (prompt &optional default _history)
...
(defmacro magit2-bind-match-strings (varlist string &rest body)
...
(defun magit2-delete-line ()
  "Delete the rest of the current line."
  (delete-region (point) (1+ (line-end-position))))

(defun magit2-delete-match (&optional num)
...
(defun magit2-file-lines (file &optional keep-empty-lines)
...
(defun magit2-set-header-line-format (string)
  "Set the header-line using STRING.
Propertize STRING with the `magit2-header-line'.  If the `face'
...
(defun magit2--format-spec (format specification)
...
(defun magit2-kill-this-buffer ()
  "Kill the current buffer."
  (interactive)
  (kill-buffer (current-buffer)))

(defun magit2--buffer-string (&optional min max trim)
...
(defun magit2-file-accessible-directory-p (filename)
  "Like `file-accessible-directory-p' but work around an Apple bug.
See http://debbugs.gnu.org/cgi/bugreport.cgi?bug=21573#17
and https://github.com/magit2/magit2/issues/2295."
  (and (file-directory-p filename)
       (file-accessible-directory-p filename)))

(when (magit2--version>= emacs-version "25.1")
...
(when (< emacs-major-version 26)
  ...
  (defun magit2-completion-pcm--all-completions (prefix pattern table pred)
...
(defun magit2-custom-initialize-reset (symbol exp)
...
(defun magit2-hook-custom-get (symbol)
...
;;;###autoload
(defun Info-follow-nearest-node--magit2-gitman (fn &optional fork)
  (let ((node (Info-get-token
               (point) "\\*note[ \n\t]+"
               "\\*note[ \n\t]+\\([^:]*\\):\\(:\\|[ \n\t]*(\\)?")))
    (if (and node (string-match "^(gitman)\\(.+\\)" node))
        (pcase magit2-view-git-manual-method
          ...
          (_
           (user-error "Invalid value for `magit2-view-git-manual-method'")))
      (funcall fn fork))))

;;;###autoload
(advice-add 'Info-follow-nearest-node :around
            'Info-follow-nearest-node--magit2-gitman)

;; When making changes here, then also adjust the copy in docs/Makefile.
;;;###autoload
(advice-add 'org-man-export :around 'org-man-export--magit2-gitman)
;;;###autoload
(defun org-man-export--magit2-gitman (fn link description format)
...
(defun magit2--straight-chase-links (filename)
...
(if (fboundp 'with-connection-local-variables)
    (defalias 'magit2--with-connection-local-variables
      'with-connection-local-variables)
  (defmacro magit2--with-connection-local-variables (&rest body)
...
(defun magit2-message (format-string &rest args)
  "Display a message at the bottom of the screen, or not.
Like `message', except that if the users configured option
`magit2-no-message' to prevent the message corresponding to
FORMAT-STRING to be displayed, then don't."
  (unless (--first (string-prefix-p it format-string) magit2-no-message)
    (apply #'message format-string args)))

(defun magit2-msg (format-string &rest args)
...
(defmacro magit2--with-temp-position (buf pos &rest body)
  (declare (indent 2))
