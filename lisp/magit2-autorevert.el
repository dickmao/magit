;;; magit2-autorevert.el --- revert buffers when files in repository change  -*- lexical-binding: t -*-
...
(defgroup magit2-auto-revert nil
  "Revert buffers when files in repository change."
  :link '(custom-group-link auto-revert)
  :link '(info-link "(magit2)Automatic Reverting of File-Visiting Buffers")
  :group 'auto-revert
...
by setting the value to `magit2-auto-revert-repository-buffer-p'.
...
is enabled."
  ...
  :group 'auto-revert
  :group 'magit2-auto-revert
  :group 'magit2-related
  :type '(radio (const :tag "No filter" nil)
                (function-item magit2-auto-revert-buffer-p)
                (function-item magit2-auto-revert-repository-buffer-p)
                function))

(defcustom magit2-auto-revert-tracked-only t
  "Whether `magit2-auto-revert-mode' only reverts tracked files."
  :package-version '(magit2 . "2.4.0")
  :group 'magit2-auto-revert
  :type 'boolean
  :set (lambda (var val)
         (set var val)
         (when (and (bound-and-true-p magit2-auto-revert-mode)
                    (featurep 'magit2-autorevert))
           (magit2-auto-revert-mode -1)
           (magit2-auto-revert-mode))))

(defcustom magit2-auto-revert-immediately t
  "Whether Magit reverts buffers immediately.

If this is non-nil and either `global-auto-revert-mode' or
`magit2-auto-revert-mode' is enabled, then Magit immediately
...
are actually supported), then `magit2-auto-revert-immediately'
does not have to be non-nil, because the reverts happen
immediately anyway.

If `magit2-auto-revert-immediately' and `auto-revert-use-notify'
...
(defun magit2-turn-on-auto-revert-mode-if-desired (&optional file)
  (if file
      (--when-let (find-buffer-visiting file)
        (with-current-buffer it
          (magit2-turn-on-auto-revert-mode-if-desired)))
    (when (and (not auto-revert-mode)        ; see #3014
               ...
               (or (< emacs-major-version 27)
                   (with-no-warnings
                     (condition-case nil
                         (executable-find magit2-git-executable t) ; see #3684
                       (wrong-number-of-arguments t)))) ; very old 27 built
               (magit2-toplevel)
               (or (not magit2-auto-revert-tracked-only)
                   (magit2-file-tracked-p buffer-file-name)))
      (auto-revert-mode 1))))

;;;###autoload
(define-globalized-minor-mode magit2-auto-revert-mode auto-revert-mode
  magit2-turn-on-auto-revert-mode-if-desired
  :package-version '(magit2 . "2.4.0")
  :link '(info-link "(magit2)Automatic Reverting of File-Visiting Buffers")
  :group 'magit2-auto-revert
...
;; - If the user has set the variable `magit2-auto-revert-mode' to nil
;;   after loading magit2 (instead of doing so before loading magit2 or
;;   by using the function), then we should still respect that setting.
;; - If the user sets one of these variables after loading magit2 and
;;   after `after-init-hook' has run, then that won't have an effect
;;   and there is nothing we can do about it.
(defun magit2-auto-revert-mode--init-kludge ()
...
and code surrounding the definition of this function."
  (if magit2-auto-revert-mode
      (let ((start (current-time)))
        (magit2-message "Turning on magit2-auto-revert-mode...")
        (magit2-auto-revert-mode 1)
        (magit2-message
         "Turning on magit2-auto-revert-mode...done%s"
...
(if after-init-time
    ...
    (magit2-auto-revert-mode--init-kludge)
  ;; By the time the init file has been fully loaded the
  ;; values of the relevant variables might have changed.
  (add-hook 'after-init-hook #'magit2-auto-revert-mode--init-kludge t))

(put 'magit2-auto-revert-mode 'function-documentation
...
If `magit2-auto-revert-tracked-only' is non-nil (the default),
then only tracked files are reverted.  But if you stage a
previously untracked file using `magit2-stage', then this mode
notices that.
...
in the `autorevert' and `magit2-autorevert' groups.

This function calls the hook `magit2-auto-revert-mode-hook'.
...
(defun magit2-auto-revert-buffers ()
  (when (and magit2-auto-revert-immediately
             (or global-auto-revert-mode
                 (and magit2-auto-revert-mode auto-revert-buffer-list)))
    (let ((auto-revert-buffer-list-filter
           (or auto-revert-buffer-list-filter
               #'magit2-auto-revert-repository-buffer-p)))
      (auto-revert-buffers))))

(defvar magit2-auto-revert-toplevel nil)

(defvar magit2-auto-revert-counter 1
  "Incremented each time `auto-revert-buffers' is called.")

(defun magit2-auto-revert-buffer-p (buffer)
...
If there is no current repository, then return t for any BUFFER."
  (magit2-auto-revert-repository-buffer-p buffer t))

(defun magit2-auto-revert-repository-buffer-p (buffer &optional fallback)
  "Return non-nil if BUFFER visits a file inside the current repository.
The current repository is the one containing `default-directory'.
If there is no current repository, then return FALLBACK (which
defaults to nil) for any BUFFER."
  ;; Call `magit2-toplevel' just once per cycle.
  (unless (and magit2-auto-revert-toplevel
               (= (cdr magit2-auto-revert-toplevel)
                  magit2-auto-revert-counter))
    (setq magit2-auto-revert-toplevel
          (cons (or (magit2-toplevel) 'no-repo)
                magit2-auto-revert-counter)))
  (let ((top (car magit2-auto-revert-toplevel)))
    (if (eq top 'no-repo)
        fallback
...
(defun auto-revert-buffers--buffer-list-filter (fn)
  (cl-incf magit2-auto-revert-counter)
...
;;; _
(provide 'magit2-autorevert)
;;; magit2-autorevert.el ends here
